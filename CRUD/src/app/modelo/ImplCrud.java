...
public class ImplCrud implements Crud
{
    ...
    /*public Producto seleccionarProductoNOSEGURO(String codigo, String nombre) {
        Producto producto = null;
        try {
            Connection conn = this.conectar();
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM producto WHERE codigo = '" + codigo + "' AND nombre = '" + nombre + "'");
    ...
    public static void main(String[] args) {
        ...
        i.seleccionarTodo();
        // System.out.println(i.buscar(10));
        i.insertar(new Producto("01UF4","COLA",4.12));
        i.seleccionarTodo();

        Crud c = new ImplCrud();
        c.seleccionarTodo();
        c.actualizar(new Producto(1,"S001","Monitor Samsung Ultra HD",200));
        System.out.println("------------------------------------------------");
        c.seleccionarTodo();
        c.eliminar(1);
        System.out.println("------------------------------------------------");
...
//        int id = dato.nextInt();
        int id = Integer.parseInt(JOptionPane.showInputDialog(null, "Ingrese el id del prducto: "));
